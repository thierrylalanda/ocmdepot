...
<!DOCTYPE html>
<c:if test="${sessionScope.root=='EH2S' or not empty sessionScope.lien98}">
            ...
            <div class="col-md-4">
                <div class="card">
                    ...
                    <div class="card-content">
                        ...
                        <c:forEach items="${commandesNew}" var="all">
                            ...
                            <a href="gestionCommandes?action=model&model=PasserCommandes&q=${q}&isNew=3&id=${all.getId()}" title="${all.getQtotal()} produits commandé(s)" >
                                <li class="list-group-item <c:if test="${all.getId()==commandes.get(0).getLigne().getId()}">active</c:if>">
                                        ...
                                        <c:if test="${all.getQtotal()<10}">bg-primary</c:if>
                                        <c:if test="${all.getQtotal()>=10}">bg-danger</c:if>">
                                        ${all.getQtotal()}
                                    </span>
            ...
            <div class="col-md-8">
                <div class="card">
                    ...
                    <div class="card-content">
                        <div class="card-body">
                            <div class="card-text">
                                <form class="form" novalidate method="post" action="gestionCommandes?action=model&model=PasserCommandes&q=${q}&isNew=12&id=${commandes.get(0).getLigne().getId()}">
                                    <div class="form-body">
                                        <div class="row">
                                            ...
                                            <div class="form-group col-lg-3 col-sm-12">
                                                ...
                                                <select style="width: 100%" required  class="select2-size-xs block form-control round" name="client">
                                                    <c:forEach items="${clients}" var="cl">
                                                        <option <c:if test="${commandes.get(0).getLigne().getClient().getId() == cl.getId()}">selected</c:if>  value="${cl.getId()}">${cl.getNom()}</option>
                                                    </c:forEach>
                                                </select>
                                            </div>
                                            <div class="form-group col-lg-2 col-sm-12">
                                                <label for="categorie_article">Livraison
                                                    <span class="danger">*</span></label>
                                                <input type="text" value='${commandes.get(0).getLigne().getDateliv()}' class="form-control-sm form-control round singledate" name="njr"/>
                                            </div>
                                    ...
                                    <div class="form-body">
                                        <div class="row">
                                            ...
                                            <c:if test="${commande == null}">
                                                ...
                                                <div class="form-group col-lg-2 col-sm-12">
                                                    <label for="prix">Prix Divers</label>
                                                    <input type="number" id="prix"  value="${commande.getPrix()}" class="form-control-sm form-control round" name="prix"/>
                                                </div>
                                            </c:if>
                            ...
                            <table class="table table-dark table-striped table-bordered responsive dataex-colvis-restore">
                                ...
                                <tbody>
                                    ...
                                    <td class="hidden td_to_updateP"><input idC="${cmd.getId()}" class="qt_updateP  round" type="number" value="${cmd.getQuantite()}"></td>
                            ...
                            <c:if test="${sessionScope.root=='EH2S' or not empty sessionScope.lien102}">
                                <div class="form-group col-md-4">
                                    <div class="btn-group btn-sm" role="group" aria-label="Basic example">
                                        ...
                                        <a title="Enregistrer la commande" class="btn-sm btn btn-float btn-outline-warning btn-round add_commande"

<%-- 
    Document   : resolveBon
    Created on : 25 avr. 2019, 15:22:06
                        ...
                        <ul class="list-group list-group-flush" style="max-height: 600px;overflow-y: scroll">
                            <c:forEach items="${lignesAccount}" var="all">
                                <c:if test="${all.getEtat()==0}">
                                    <a href="gestioncaisse?action=model&model=addBonCommande&q=${q}&isNew=2&id=${all.getId()}&forpaiement=0" title="Total  Bon ${all.getMontantNet()}" >
                                        <li class="list-group-item <c:if test="${all.getId()==sessionScope.ligneEncour.getId()}">active</c:if>">
                                            ...
                                            <c:if test="${all.getNumeroBon() != null}">
                                                ${all.getNumeroBon().getFournisseur().getNom()}
                                            </c:if>
                                            <c:if test="${all.getNumeroBon() == null}">
                                                <c:if test="${all.getFournisseur() != null}">
                                                    ${all.getFournisseur().getNom()}
                                                </c:if>
                                            </c:if>
                                            <f:formatDate value="${all.getDateCreate()}" type="BOTH" dateStyle="MEDIUM" pattern="dd/MM/yyy"/>
                                        </li>
                                    </a>
                                </c:if>
                            </c:forEach>
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title" id="from-actions-center-dropdown">Bon de ${ligne.getNumeroBon().getFournisseur().getNom()}</h4>
                    <a class="heading-elements-toggle"><i class="fa fa-ellipsis-v font-medium-3"></i></a>
                    ...
                    <div class="card-body">
                        <div class="card-text">
                            <c:if test="${ligne.getEtat()==0}">
                                <form class="form" novalidate method="post" action="gestioncaisse?action=model&model=reglementAccount&q=${q}&isNew=${isnew}${id}&ligne=${ligne.getId()}&forpaiement=0">
                                    <div class="form-body">
                                        <div class="row">
                                            <div class="form-group col-lg-2 col-sm-12">
                                                <label for="quantite">Avance
                                                    <span class="danger">*</span></label>
                                                <input type="number" required id="quantite" value="${ligne.getMontantRestant()}" min="0" class="form-control-sm form-control round" name="avance"
...
<!--                                    <button type="button" title="Modifier" class="btn-sm btn btn-float btn-outline-primary btn-round btn-link-update" name="id" action="gestioncaisse?action=model&model=reglementAccount&q=${q}&isNew=2"><i class="fa fa-edit"></i></button>-->
                        ...
                        <div class="text-center bg-dark title_table" style="color:white">Liste des versements concernant le fournisseur ${ligne.getNumeroBon().getFournisseur().getNom()}</div>
                        <br>
                        <table class="table table-dark table-striped table-bordered responsive dataex-colvis-restore">
                            ...
                            <tbody>
                                ...
                                <c:forEach items="${ligne.getSortietList()}" var="cmd">
                        ...
                        <a class="btn btn-outline-secondary btn-sm round" href="gestioncaisse?action=model&model=init&q=commande/caisse/allDetteFournisseur&isNew=0"><icon class="fa fa-arrow-left"></icon></a>
                    </div>
                </div>
            </div>
        </div>

<%-- 
    Document   : addsite
    Created on : 20 oct. 2018, 10:34:59
    Author     : messi
--%>
...
<c:if test="${site != null}">
    ...
    <c:set var="id" value="&site=${site.getId()}"></c:set>
</c:if>
<c:if test="${site == null}">
    <c:set var="isnew" value="${0}"></c:set>
</c:if>
<div class="modal fade in text-left <c:if test="${site != null}">autoshow</c:if>" data-backdrop="static" id="bootstrap" tabindex="-1" role="dialog" aria-labelledby="myModalLabel35"
     aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header bg-primary">
                    <h3 class="modal-title" id="myModalLabel35">Ajouter un site</h3>
                <c:if test="${site == null}">
            ...
            <div class="modal-body">
                <form method="post" action="administration?q=${q}&action=model&model=site&isNew=${isnew}${id}" class="" novalidate>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select style="width: 100%"  id="societe"  class=" select2-size-xs block form-control round required region_by_societe"  name="societe">
                                            ...
                                            <c:forEach items="${societes}" var="soc">
                                                <option <c:if test="${soc.getId()==site.getRegionid().getSociete().getId()}">selected</c:if> value="${soc.getId()}"> ${soc.getNom()}</option>
                                            </c:forEach>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                                ...
                                <div class="form-group">
                                    ...
                                    <select style="width: 100%" required id="societe_region" class="select2-size-xs block form-control round societe_region" name="region">
                                        ...
                                        <c:forEach items="${regions}" var="reg">
                                            <option <c:if test="${reg.getId()==site.getRegionid().getId()}">selected</c:if> value="${reg.getId()}"> ${reg.getName()}</option>
                                        </c:forEach>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" value="${site.getName()}"  class="form-control-sm round form-control" required id="firstName" name="nom">
                                </div>
                            </div>
